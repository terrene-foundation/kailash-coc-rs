...
// ────────────────────────────────────────────────────────────────
// `.coc-obsoleted` emission (loom#932 — engine-owned, deterministic)
// ────────────────────────────────────────────────────────────────
//
// Downstream consumers learn which paths to purge from THEIR trees via a
// slim `.claude/.coc-obsoleted` file (sync-manifest.yaml itself is never
// distributed, so this is the ONLY channel). Historically the coc-sync
// AGENT wrote this file in bash (Step 4.5(b)): it re-derived the plan in a
// SECOND shell invocation, reading a `/tmp/…-$$-…json` keyed on the WRITING
// shell's PID; a cross-shell `$$` mismatch made the read silently absent, so
// the rename-orphan third source could be dropped without a trace (loom#633
// R1). The emit now lives HERE — from the SAME `plan` object that drove the
// purge loops — making it deterministic and unit-testable. The agent MUST
// NOT also write the file (double-write); it only verifies the engine output.
//
// Content = union(obsoleted, use_obsoleted, overlay_orphans):
//   - obsoleted:       cross-era orphan dirs (BUILD + USE lanes)
//   - use_obsoleted:   BUILD-internal artifacts (USE lane only)
//   - overlay_orphans: THIS variant's rename-orphans (loom#633) — a variant
//     overlay that RENAMES the global it replaces strands the global-named
//     file in every downstream consumer that synced the old name; only this
//     record drives the downstream purge. Per-variant, so py/rb (which keep
//     the global legitimately) contribute an empty third source.
//
// USE lane only: a BUILD repo is the SDK SOURCE — nothing pulls COC artifacts
// FROM it via /sync-from-template — so no contract file is generated (matching
// commands/sync-to-build.md).

const COC_OBSOLETED_PATH = ".claude/.coc-obsoleted";

const COC_OBSOLETED_HEADER = [
  "# COC obsoleted-paths manifest — read by downstream /sync-from-template.",
  "# Generated by sync-tier-aware.mjs from sync-manifest.yaml::obsoleted + use_obsoleted",
  "# + this variant's rename-orphans (plan.overlay_orphans, loom#633).",
  "# One path per line. Trailing slash means directory. Lines starting with # are comments.",
];

/**
 * Order-preserving de-dup of the three obsoleted sources into the emitted
 * union. `obsoleted` and `use_obsoleted` are disjoint by manifest design and
 * the rename-orphans are renamed globals, but de-dup makes the output
 * deterministic regardless of accidental overlap. Empty / non-string entries
 * are dropped (a blank line in the emitted file carries no purge contract).
 */
function cocObsoletedUnion(obsoleted, useObsoleted, overlayOrphans) {
  const seen = new Set();
  const out = [];
  for (const p of [
    ...(obsoleted || []),
...
/**
 * Pure content composer — header + one path per line, trailing newline.
 * Side-effect-free so the header+path-per-line format contract is
 * unit-testable without touching the filesystem.
 */
function composeCocObsoletedContent(paths) {
  return [...COC_OBSOLETED_HEADER, ...paths].join("\n") + "\n";
}

/**
 * Emit `<dir>/.claude/.coc-obsoleted` from the already-built plan. Every
 * union path is routed through `safeJoinUnder(dir, p)` FIRST — a `..` /
 * absolute / `.`-equivalent entry THROWS (fail-loud, the SAME containment
 * contract the purge branches enforce) rather than being silently dropped or
 * escaping the target. A malformed entry written verbatim into the file would
 * drive a downstream consumer's purge to escape too, so the gate protects the
 * contract's transitive reach, not just this write. Returns the emitted union
 * (for result reporting + tests); writes nothing on dryRun. Throws on a
 * containment violation — the caller (executePlan) converts that to fail(1).
 */
function emitCocObsoleted(plan, dir, dryRun) {
  const union = cocObsoletedUnion(
    plan.obsoleted,
    plan.use_obsoleted,
    plan.overlay_orphans,
  );
  for (const p of union) {
    // loom#932 R1 — emit↔consumer contract parity. `safeJoinUnder` only
    // rejects entries whose RESOLVED path escapes `dir`, so an internal-`..`
    // like `.claude/a/../b` (which resolves back UNDER dir) would slip through
    // the engine but be REJECTED by the downstream #931 string guard — the two
    // contracts would disagree on the accepted set. `rejectUnsafePurgeEntry`
    // rejects ANY `..` segment (plus absolute / `.`-self), matching the
    // downstream guard's `..`/absolute/`.`-self arms. (The downstream #931
    // guard is additionally stricter on a trailing `/.` shape (`foo/.`) — a
    // fail-safe divergence, unreachable for loom-authored clean paths.)
    // Throw → fail-loud (no partial file).
    const why = rejectUnsafePurgeEntry(p);
    if (why) throw new Error(`unsafe .coc-obsoleted entry: ${why}`);
    safeJoinUnder(dir, p); // throws on `..` / absolute / self (resolved-escape belt)
  }
  const dest = safeJoinUnder(dir, COC_OBSOLETED_PATH);
  if (!dryRun) {
    fs.mkdirSync(path.dirname(dest), { recursive: true });
    safeWriteTextSync(dest, composeCocObsoletedContent(union));
  }
  return union;
}
  ...
  try {
    gitCommonDir = execFileSync(
      ...
      ["-C", dir, "rev-parse", "--git-common-dir"],
      {
        ...
        maxBuffer: MAX_GIT_BUFFER, // loom#888 — uniform across every capture site (output is bounded here, but keeps the constant's "every site" claim true)
      },
    ).trim();
  } catch {
  ...
  try {
    untracked = execFileSync(
      ...
      ["-C", dir, "ls-files", "--others", "--exclude-standard", "-z"],
      {
        encoding: "utf8",
        stdio: ["ignore", "pipe", "ignore"],
        maxBuffer: MAX_GIT_BUFFER, // loom#888 — 1MB default ENOBUFS on large untracked sets
      },
    )
      .split("\0")
      .filter(Boolean);
...
// loom#888 — explicit maxBuffer for git calls that CAPTURE output.
// Node's default execFileSync maxBuffer is 1MB; a `git ls-files --others`
// scan over a genuinely large untracked-and-not-ignored set (e.g. many
// mounted worktrees inflating the tree) exceeds it and throws
// `spawnSync git ENOBUFS`, failing the sync/dry-run (and Validator 17's
// end-to-end probe). 64MiB is generous headroom for even a pathological
// untracked path-list without unbounding memory. Applies to every
// execFileSync/runGit site that captures git stdout in this file.
...
function defaultGitRunner(args, opts) {
  // loom#888 — default an explicit maxBuffer so a large captured output
  // (e.g. `check-ignore --stdin` echoing a large ignored subset) does not
  // hit Node's 1MB default and throw ENOBUFS. A caller-supplied maxBuffer
  // still wins.
  return execFileSync("git", args, { maxBuffer: MAX_GIT_BUFFER, ...opts });
}
...
function buildPlan(manifest, target, templateFilter, mode = "use") {
  ...
  // Both obsoleted sources parsed unconditionally: the lane picks ONE for
  // the PURGE (below), but the USE-lane `.coc-obsoleted` EMIT (loom#932)
  // unions BOTH plus this variant's rename-orphans, so both must ride on the
  // plan regardless of lane.
  const obsoletedList = parseList(sliceBlock(manifest, "obsoleted"));
  const useObsoletedList = parseList(sliceBlock(manifest, "use_obsoleted"));
  const purgeList = isBuild ? obsoletedList : useObsoletedList;
  ...
  return {
    ...
    // loom#932 — both obsoleted sources carried for the USE-lane
    // `.coc-obsoleted` emit (union of these two + overlay_orphans). The
    // engine, not the coc-sync agent, now writes the downstream contract file.
    obsoleted: obsoletedList.slice(),
    use_obsoleted: useObsoletedList.slice(),
...
function executePlan(plan, outOverride, dryRun) {
  ...
  for (let i = 0; i < plan.templates.length; i++) {
    ...
    const result = {
      ...
      purged: [],
      // loom#932 — the union of paths written to `.claude/.coc-obsoleted`
      // (USE lane only). Repo-relative paths, disclosure-clean for --json.
      coc_obsoleted: [],
    ...
    // loom#932 — engine-owned `.coc-obsoleted` emit, co-located with the
    // purge loops above so it consumes the SAME plan. USE lane only: a BUILD
    // repo is the SDK SOURCE, nothing pulls COC FROM it, so no contract file
    // (commands/sync-to-build.md). Replaces the coc-sync agent's Step-4.5(b)
    // bash, whose second-shell `$$` re-derivation could silently drop the
    // rename-orphan source (loom#633 R1). Containment-guarded (CRIT-1 parity)
    // — a `..`/absolute obsoleted entry throws → fail-loud, never a partial file.
    if (plan.mode !== "build") {
      try {
        result.coc_obsoleted = emitCocObsoleted(plan, dir, dryRun);
      } catch (e) {
        fail(1, `.coc-obsoleted emit refused: ${e.message}`);
      }
    }
    ...
    if (!dryRun) {
      const emitted = [
        ...
        // loom#932 — the engine-emitted `.coc-obsoleted` (emitCocObsoleted
        // above) is itself a written artifact, so a consumer .gitignore that
        // swallows the purge-contract file MUST be caught by the loom#676 gate.
        // Mode-guarded IDENTICALLY to the emit (`plan.mode !== "build"`): the
        // BUILD lane never emits the file, so it must not expect it either.
        ...(plan.mode !== "build" ? [COC_OBSOLETED_PATH] : []),
      ].filter(Boolean);
...
function verifyConsistency(plan, outOverride) {
  ...
  for (let i = 0; i < plan.templates.length; i++) {
    ...
    // loom#932 — mirror the executePlan emit: the engine-emitted
    // `.coc-obsoleted` is an emitted artifact on the USE lane, so verify must
    // pre-flight whether the target's .gitignore would swallow it too. Guarded
    // IDENTICALLY to the emit (`plan.mode !== "build"`) so verify stays a
    // superset of execute (both register it, mode-guarded).
    if (plan.mode !== "build") inPlanDestsRel.push(COC_OBSOLETED_PATH);
...
export {
  ...
  safeJoinUnder,
  safeReadSync,
  // loom#932 — engine-owned `.coc-obsoleted` emit
  cocObsoletedUnion,
  composeCocObsoletedContent,
  emitCocObsoleted,
  COC_OBSOLETED_HEADER,
  COC_OBSOLETED_PATH,

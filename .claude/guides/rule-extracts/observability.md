...
Trust Posture Wiring: Severity `halt-and-report` (cc-architect `/codify` sweep against new scanners that grep `*.log` without an `EXCLUDED_FILES` constant) / `advisory` (runtime hook). Grace 7 days. Detection: cc-architect greps `.claude/hooks/**/*.js` for `find … -name '*.log' … grep -HnE 'WARN|ERROR|FAIL'` lacking an adjacent `EXCLUDED_FILES` constant. **Audit fixtures are OWED and NOT YET AUTHORED.** This line previously asserted they were at `.claude/audit-fixtures/log-triage-gate/`; that directory has never existed on any ref, so the citation asserted a fact that was never true. The debt is real rather than cosmetic: `.claude/hooks/log-triage-gate.js` is a LIVE registered hook carrying the `EXCLUDED_FILES` constant, so `cc-artifacts.md` Rule 9 owes it fixtures — this is not a Phase-2 deferral that fixtures may lag. Authoring them is a shard of its own: the hook exports nothing, so a fixture harness must either drive it as a subprocess over a temp tree with planted `.log` files or gain an export surface, and a fixture set that cannot discriminate the `EXCLUDED_FILES` behaviour specifically would be worse than none (`instrument-discipline.md` MUST-3). Recorded here rather than allowlisted, so the gap stays visible. Origin: PR #1163 commit 36b8ca3d (2026-05-28).

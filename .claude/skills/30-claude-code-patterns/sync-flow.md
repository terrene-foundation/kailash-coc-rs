...
7a. **Reconcile settings.json deny-rule FORM (MUST — additive, never overwrite)**: run `node .claude/bin/reconcile-settings-deny.mjs --write .claude/settings.json` on THIS consumer's own settings.json. It rewrites every `permissions.deny` entry of the form `Write(<x>)` / `NotebookEdit(<x>)` to `Edit(<x>)` and collapses `Write(x)`+`Edit(x)`+`NotebookEdit(x)` to one `Edit(x)`. It touches the deny-rule FORM ONLY — NEVER `permissions.allow`, the consumer's `hooks[]` (paths included, so it runs AFTER step 7's hook-path normalization without undoing it), or any other key — and leaves an already-clean file byte-for-byte unchanged (idempotent; a MISSING settings.json is a benign no-op). A changed file is re-serialized, so formatting of unrelated blocks may normalize (values preserved). Why: Claude Code no longer matches `Write(<path>)` / `NotebookEdit(<path>)` deny rules — only `Edit(<path>)` covers file-editing tools; a consumer still carrying the stale form errors on every CC session init AND leaves its guarded state files un-denied. This is the leg that self-heals already-deployed consumers.
...
- **GitHub** (`github`): the consumer lane has EXACTLY TWO methods — the HUMAN-GATED inbox PR → `createUpflowPR(transport, {repoRef, head, base, title, body})`, and the no-fork Route-A issue → `createUpflowIssue(transport, {repoRef, title, body, labels?})` (both on `.claude/hooks/lib/vcs-github-adapter.js`; the agent-followed CLI equivalents are `gh pr create` / `gh issue create`). **There is NO third consumer-lane method: the upflow OPENS and STOPS** (`upstream-issue-hygiene.md` MUST-4). `completeUpflowPR` (→ `gh pr merge`) is **NOT on this lane at all** — it is the UPSTREAM MAINTAINER's primitive, to be called only on the maintainer's OWN repo during `/sync-from-downstream`. It DERIVES that identity itself from `process.cwd()` (`hooks/lib/upflow-self-repo.js`): the **live git remote is the sole authoritative source**, and `.claude/VERSION::repo` is a **refuse-only cross-check** — present-and-disagreeing REFUSES, but it can never SUPPLY the identity, so a forged `VERSION` file is powerless. No identity, `cwd`, or deriver value is taken off the caller's DESCRIPTOR (`deriveSelfRepoRef` does take a `cwd` argument, which both adapters hardcode to `process.cwd()`; what was removed is the caller's ability to SUPPLY one) — removing those seams eliminated a trivially-forgeable descriptor field. It fails closed against any other repo, against a repo whose `VERSION` contradicts its remote, against one whose remote's HOST is not a recognized host for the provider being driven (an internal mirror of an upstream cannot authorize a merge on the real one), and against one whose remote cannot be derived at all. **Stated at its true strength:** it refuses any completion whose target does not match the identity derived from the working tree the process runs in — which CLOSES the accident class (a consumer following stale prose is refused before the transport fires) and raises the cost of a deliberate act. It is NOT a boundary against a caller that can choose its own working directory (`process.cwd()` is whatever launched the process), and it cannot be; the full covered-vs-uncovered statement lives in `upstream-issue-hygiene.md` MUST-4. A consumer that reaches for it is BLOCKED by the rule AND refused by the adapter.
- **Azure DevOps** (`azure-devops`): the SAME TWO consumer-lane methods on `.claude/hooks/lib/vcs-azure-adapter.js` — PR via `pullrequests`, the Route-A fallback as a **work-item** whose type is `getAdoWorkItemType()` (default `Task`, G-F-3). Its `completeUpflowPR` carries the identical own-repo-only fence as the GitHub adapter (`security.md` § Enforcement-Surface Parity — the fence landed at BOTH providers in the same change, so the un-fenced provider cannot become the bypass); it derives `org` by parsing the ADO remote forms and REFUSES when the remote is not ADO-shaped, so org, project, and repo are all compared against derived values rather than against themselves. Every ADO upflow result carries `unverified: true` (no live ADO test org — G-F-4 gate, same posture as the deploy half; `rules/verify-resource-existence.md` MUST-2). **G-F-1 (security):** the ADO work-item adapter sets ONLY `System.Title` + `System.Description` and NEVER auto-populates the work-item disclosure surfaces (`System.AreaPath` / `IterationPath` / `Tags` / `AssignedTo`) — they default to the project root, carrying no consumer identity. The minimal, fixed field set IS the structural neutralization.
...
**The Step-7c stop-point (MUST).** Step 7c is COMPLETE when the PR (or Route-A issue) is OPEN. The PR URL **is** the deliverable and the handoff; report it and stop. The consumer MUST NOT merge, complete, admin-merge, enable auto-merge on, or directly push to the upstream — MUST-1's gate authorizes SUBMISSION, never COMPLETION (`upstream-issue-hygiene.md` MUST-4). "The cascade hasn't happened until it's merged" is BLOCKED reasoning: the cascade is the upstream's `/sync-from-downstream` ingest → Gate-1 → `/sync-to-use`, and a self-merged offer skips the scrub, the untrusted-data review, the dedup, and the lane check while still producing an `ingest_disposition` receipt attesting they ran.
...
**Corollary for the Step-0b freshness probe (2026-08-04).** Because the worktree is cut from the target's REMOTE main and the engine is driven with `--out <scratch>` (`sync-tier-aware.mjs`'s `resolveTemplateDir`/`resolveBuildDir` return the override before resolving the target's checkout), the target's local `main` and working tree are NOT inputs to the delivery. `check-sync-freshness.mjs` therefore scopes severity by ROLE: loom halts on any divergence (§0b(i) below), while a target that is merely BEHIND is `ADVISORY` at exit 0 — halting there stops a cycle for a condition with no effect on its outcome, and the old remediation prescribed `git reset --keep origin/main` against a sibling developer's clone, which is the working-tree move THIS section forbids. What still HALTs on a target is LOCAL-ONLY COMMITS (`ahead`/`diverged`) or an ancestry that cannot be established: a worktree cut from `origin/main` cannot see unpushed commits, so the delivery is computed against a base that omits them and the pre-merge delivered-removal review reads them as intentional deletions. Note the ONE in-cycle reader of a target's LIVE checkout that survives this: `tools/verify-overlays.sh` (§(iv) below) — attribute its `deployed-missing`/drift rows on an ADVISORY target to the lag before treating them as findings.
...
### Pre-merge delivered-removal review (MUST — loom#1549)

Gate-2 DELETES: the obsoleted purge is part of the same atomic apply as the adds (`journal/0339`), so every Gate-2 PR carries a REMOVAL SET — and it is the half no existing gate reads. The engine's `--verify` structurally cannot: it asserts the delivery matches the SOURCE, and a target-originated artifact is absent from source by definition, so removing it is precisely what `CONSISTENT ✓` looks like. `coc-sync.md` Step 8's byte-stability snapshot covers only the manifest-declared preserve classes (`project_owned_preserve` + its variant overlays); a target-authored artifact outside that set is deleted with nothing asserting on it. Before merging ANY Gate-2 PR, enumerate the removal set and adjudicate every row.

```bash
# DO — the authoritative removal check; a NON-EMPTY result is the finding
head=$(gh pr view <N> --repo <org>/<repo> --json headRefOid -q .headRefOid)
git diff --diff-filter=D --name-only "$(git merge-base origin/main "$head")" "$head"
# no local clone? the PAGINATED REST form is equivalent (`status`, not `changeType`)
gh api --paginate repos/<org>/<repo>/pulls/<N>/files -q '.[]|select(.status=="removed").filename'
# DO NOT — `gh pr view --json files` SILENTLY TRUNCATES AT 100 files, nothing on stderr
gh pr view <N> --json files -q '.files[]|select(.changeType=="DELETED").path'
```

**Read it as an instrument (`instrument-discipline.md` MUST-1).** A NON-EMPTY result means this sync removes those paths from the target, and every row MUST be traced to an `obsoleted:` / `use_obsoleted:` entry or a declared overlay-orphan before merge — an untraceable row is the loom#1549 class and BLOCKS the merge. An EMPTY result is evidence for "this sync deletes nothing" ONLY because a non-empty result was reachable by the same command on the same PR; that reachability is exactly what the truncating form cannot offer, which is why an empty file-list from it means nothing at all.

**`gh pr view --json files` is not deletion-blind — it is TRUNCATION-blind, which is worse.** It does carry `changeType: "DELETED"`, and on a small PR its DELETED set is exactly `git diff --diff-filter=D` (set-identical on loom#425, 26 = 26). But its GraphQL query caps at 100 files and does NOT paginate: on loom#1366 it reported `changedFiles: 138` beside a 100-entry `files` array — silently dropping 38 paths including `.claude/rules/state-file-write-guard.md`, with an empty stderr. A Gate-2 sync PR is routinely far past 100 files, so it is the CAP, not the schema, that makes a deleted artifact read as untouched — and the same cap hides ADDs and MODIFYs.

**BLOCKED rationalizations:** "the PR file list showed those paths untouched" / "the engine's `--verify` came back CONSISTENT ✓" / "the purge only removes obsoleted paths, that is its contract" / "the deleted count in the PR body looked small" / "the preserve-set snapshot would have caught it" / "the target can restore anything we removed by mistake".

**Why:** occurrence 1 of loom#1549 merged BECAUSE a truncated file list read clean; occurrences 2 and 3 were caught by the TARGET, never by loom. A deleted governance rule reverts an enforcement gate with no error to see — the artifact's absence produces nothing but an obligation that quietly stops being checked.

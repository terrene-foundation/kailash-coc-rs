...
- **Detection mechanism:** TWO tiers, neither over-claimed. **Structural (library-boundary refusal, NOT a hook):** the `completeUpflowPR` fence in the `vcs-github-adapter` + `vcs-azure-adapter` modules (module NAMES, never `hooks/lib/` PATHS — extract § Adapters: MODULE NAME) derives self-identity via `upflow-self-repo` and refuses, before the transport fires, on an underivable / contradicting / unrecognized-host / non-self identity; its fixtures are CI-run via `ci-audit-fixtures.json` **AT LOOM ONLY** (merge-blocking there: mutable settings, re-measure). **At a USE template or downstream consumer this tier is COMMITTED-FIXTURES, MANUALLY DRIVEN — not a live gate.** `.claude/test-harness/**` is never-synced to USE (measured on the rs lane: 298 files, every one `skip`, and `ci-audit-fixtures.json` itself `skip:exclude`), so no consumer receives the runner that would execute them. Stating the loom-side CI fact unqualified told every consumer a merge-blocking tier existed where none does — the exact over-claim class this block's own "neither over-claimed" header promises against. **Semantic (Phase 1, gate-review):** reviewer / cc-architect inspect any session that ran a Step-7c upflow. Probes `.claude/test-harness/probes/upstream-issue-hygiene.probes.json`; candidates + answer keys `.claude/audit-fixtures/upstream-issue-hygiene/`. **NO Phase-2 detector is committed, scheduled or promised, and this block declares no deferral** — an undated promise is permanent-by-default, which `deferral-registry-locality.md` BLOCKS; whoever builds one registers a dated entry in `.claude/test-harness/phase2-deferrals.json`, fixtures in the same change. Residuals + candidate surfaces: extract § Detection mechanism.

...
**Empirical-claim status (2026-05-08, post F-1.5):** F-1 (source (d), literal user quote in prompt) and F-1.5 (sources (a)/(b)/(c)/(e), anchor in materialized brief/journal/spec) ablation cycles measured the rule's behavioral signal. Result: rule produces a ~33pp differential on **formal value-rank-list shape** (F-1 source-(d): with-rule 5/6, without-rule 3/6) and **0pp on substantive selection** when the user-anchored source is salient anywhere (F-1.5 sources (a/b/c/e): with-rule 4/4, without-rule 4/4 — model spontaneously anchors on the materialized resource without rule prompting). The rule's MUST clauses are unaffected by this measurement; the runtime detection layer (hooks `detectStreetlightSelection`, `detectDeferralWithoutValueAnchor`, `detectDeferredItemPickupWithoutRevalidation`, `detectGhIssueCloseAsNotPlanned`) is the load-bearing enforcement surface, and shape teeth at the prose layer is what the F-1 differential measures. The rule's substantive defense surface (Failure-A reframing pattern: agent reframes user-approved spec step using a feedback memory as authority, with NO in-prompt anchor and NO declared anchor-status for the lower-value option) remains unmeasured by F-1/F-1.5; F-2.0 (issue #100) is the deferred follow-up that builds fixtures actually triggering Failure-A. Full results: `journal/0055-DISCOVERY-…f1-empirical-signal.md`, `journal/0056-…f1-rerun…`, `journal/0057-…f1-rubric-tightening…`, `journal/0058-…f1-s3-fix-verified…`, `journal/0059-DISCOVERY-value-prioritization-f1-5-empirical-results.md` (issue #86 closed 2026-05-08).

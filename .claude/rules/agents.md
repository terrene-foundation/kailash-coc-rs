...
An agent that SUCCEEDS and returns nothing is the same zero evidence as one that errors (§ Redteam Reviewer Dispatch), and is MORE dangerous because **every surface reports success**. **(1) SPAWN CONTRACT** — when the orchestrator needs the result back it MUST spawn WITHOUT a `name`: a named agent is a teammate carrying NO `toolUseId`, so no tool call awaits it and its final message has no return path (`run_in_background: false` does NOT override this; `name` also shadows `subagent_type`). Naming is permitted ONLY when the prompt instructs reporting via `SendMessage({to: "main"})`; pairing a `name` with "your final message IS the return value" is BLOCKED. **(2) DELIVERY GATE** — neither a lifecycle/idle notification NOR a payload that is a STATUS FRAGMENT is a delivery signal. A fragment is defined by ANNOUNCING WORK STILL TO COME rather than stating a result — NOT by brevity: a terse complete verdict ("CLEAN — no findings") IS a delivery and MUST NOT be re-run as one. Read what is IN the result, never merely THAT a result exists; a fragment is zero evidence and MUST NOT be scored as a clean lane. **(3) RECOVERY, BOUNDED** — recovery keys on the RETURN PATH (the `toolUseId`/`name` axis of (1)), not on guesswork about what the agent wrote: a NAMED agent's already-written report MUST be read from its transcript, never re-requested; an UNNAMED lane that stalled before writing MUST be resumed via `SendMessage`, never re-dispatched. The resume is bounded — **ONE per lane** (a second fragment routes to transcript-recovery or re-dispatch, recorded); it MAY release only a sub-goal the AGENT assigned itself, and a BRIEF item named as the blocker MUST be escalated, never marked optional; and the resumed payload counts as a delivery only if it addresses each brief item or names it UNRESOLVED — **UNRESOLVED items are open findings and do NOT count toward a clean round**. Measurement, the untested named-and-stalled quadrant, the live mode-1 hypothesis, BLOCKED corpora, transcript-recovery: `skills/30-claude-code-patterns/agent-result-delivery.md`.

**Why:** the report is written in full then discarded silently, so the orchestrator pays the whole cost again re-deriving work sitting complete on disk. The fragment variant is worse: a populated result field makes a fan-out read as fully delivered, so the missing lane is invisible at exactly the moment convergence is declared.
...
Applies to the **§ Agent-Result-Delivery** clause ONLY (added 2026-08-13, USE-template origination; DELIVERY GATE extended + RECOVERY half added 2026-08-14 after the class recurred twice in one session); ships canonical-8-field-compliant. Grandfather + precedent: guide § Clause-Scoped Wiring Precedent.

- **Severity:** `halt-and-report` at gate-review (reviewer at `/implement` + cc-architect at `/codify` confirm every relied-on payload STATED A RESULT rather than announcing work to come — "returned a payload" is the test this clause declares too weak and does NOT satisfy the gate — that no `name` was paired with the task-return-contract instruction, and that any resume stayed inside the (3) bounds: one per lane, self-assigned sub-goal only, UNRESOLVED items carried as open findings); `advisory` at the hook layer per `hook-output-discipline.md` MUST-2. The `name` field is structurally present in the `PreToolUse` input, but the other half of the predicate — whether the prompt instructs `SendMessage` reporting — is decidable only LEXICALLY over prompt prose, and MUST-2 bars `block` on lexical evidence regardless of how good the matcher gets. A better adjudicator would NOT unlock `block`.
- **Grace period:** 7 days from clause landing (2026-08-13 → 2026-08-20); the 2026-08-14 DELIVERY-GATE extension + RECOVERY half open their own 7-day window (2026-08-14 → 2026-08-21) on their own new obligations. Recorded rather than folded into the original window: the two 2026-08-14 instances occurred BEFORE these halves existed, so they are the rule's evidence, not violations of it.

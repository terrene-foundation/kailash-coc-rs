...
```rust
// DO — single helper (method on an ApiKeyConfig holding the valid-key hashes); always walks the full list
let ok = api_key_config.validate_key(token);

// DO NOT — .any() short-circuits, leaks match position via timing
let ok = valid_keys.iter().any(|k| bool::from(token.ct_eq(k)));
```

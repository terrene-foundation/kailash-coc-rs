...
DataFlow's input sanitizer (the dataflow package's src/dataflow/core/nodes.py::sanitize_sql_input is a defense-in-depth display-path safety net, NOT the primary SQLi defense. Parameter binding (`$N` / `%s` / `?`) is the primary defense — see § Parameterized Queries above.

...
consultant /codify finding:
  ...
  capability gap / bug → human-gated BUILD issue (scrubbed) (manual route; auto-draft is G3.4)
...
**The auto-classifier (artifact vs capability vs bug) is the G3.4 implementation gap — NOT shipped.** Today the consultant (or a human gate) decides the lane; the consultant-facing tooling that AUTO-classifies a `/codify` finding and auto-drafts the correct PR/issue is `/implement` work (`specs/03 §4` G3.4), deliberately deferred. This subsection ships the routing **doc contract** (which TYPE goes to which lane, and the never-talk-to-an-engineer invariant) — it does NOT assert an auto-classifier exists. The upflow's gh-vs-ADO provider abstraction for the PR/issue write-surface is the separate gap **G-F** (`specs/05 §3`).
...
**Detection (gate-reviewed, LLM-judgment — NOT a mechanical sweep):** an O1 origination is a `/codify`, so the standing cc-architect review every `/codify` deploys (per `cc-artifacts.md` Rule 6) gate-reviews it — reading the journal `DECISION` receipt to confirm BY JUDGMENT that it cites a specific version+clause AND states the one-sentence derivation, and halting on an uncited or non-governing edit. This is gate-level LLM-judgment, not a mechanical detector and not necessarily the `self-referential-codify.md` multi-agent gate (which fires only when the compliance artifact ITSELF is a codify-governing surface; a typical compliance rule governs code behavior, so it is outside that allowlist). A mechanical O1-citation check is unbuilt Phase-2 work, named as such — not claimed here.
...
edit .claude/rules/<compliance-rule>.md  "per ISO 27001"  # BLOCKED — bare name, no clause, no derivation = loophole
```

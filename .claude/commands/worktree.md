...
- The worktree is **durable** — do NOT delete it between tasks (unlike agent-wave worktrees). "Durable" means not deleted BETWEEN tasks; it never means permanent. When fully done: `git worktree remove "$wt_path"` — **never `--force`**. A bare `remove` REFUSES a dirty tree, and that refusal is the safety net working; confirming a clean tree and THEN forcing is the check-then-clobber TOCTOU (`rules/worktree-isolation.md` Rule 8), because the state can change between the check and the removal, and unstaged + untracked-not-ignored work has NO reflog. To classify the whole forest rather than one tree, run `node .claude/bin/worktree-reap.mjs` (report-only; `--apply` reaps only what a ref preserves).

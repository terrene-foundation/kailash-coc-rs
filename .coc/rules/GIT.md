...
- **Regression-within-grace:** routes the CUMULATIVE MUST-4 path — NO dedicated emergency trigger key (a transcript-history judgment property does not warrant an instant-drop key). Named deviation from the canonical key-per-clause shape, recorded here per `trust-posture.md` Rule 8.

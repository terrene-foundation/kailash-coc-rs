...
- **Downstream USE repos** (consumer projects): `/codify` writes to `.codex/agents/project/` and `.codex/skills/project/` and stays LOCAL. No `.proposals/latest.yaml` is created; no upstream flow. These artifacts are preserved across `/sync-from-template` runs by the "Project-specific" rule above.

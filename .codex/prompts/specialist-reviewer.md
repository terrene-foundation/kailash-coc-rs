...
After invoking `/prompts:specialist-reviewer`, your context now contains the operating specification below. Read the user's task and respond as the reviewer specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): invoke `/prompts:specialist-reviewer`, then provide your task in the same session.

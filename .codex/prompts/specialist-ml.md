...
After invoking `/prompts:specialist-ml`, your context now contains the operating specification below. Read the user's task and respond as the ml specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): invoke `/prompts:specialist-ml`, then provide your task in the same session.

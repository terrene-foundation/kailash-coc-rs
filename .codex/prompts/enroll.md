...
per-operator gitignored layer. Procedure detail lives in `.codex/skills/44-enroll/SKILL.md` per
...
Invoke `/whoami --register` (`.codex/prompts/whoami.md` § `/whoami --register`). It collects
...
B1's roster-write path is `.codex/prompts/whoami.md` (the `--register` subcommand) + the schema
...
lives in `.codex/skills/44-enroll/SKILL.md`.

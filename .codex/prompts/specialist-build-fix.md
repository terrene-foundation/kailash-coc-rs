...
After invoking `/prompts:specialist-build-fix`, your context now contains the operating specification below. Read the user's task and respond as the build-fix specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): invoke `/prompts:specialist-build-fix`, then provide your task in the same session.
